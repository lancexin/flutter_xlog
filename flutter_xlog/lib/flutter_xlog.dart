...
class XLog {
  ...
  /// flush in-memory log buffer to disk.
  ///
  /// **isSync** when *true*, blocks until the buffer is fully written; when
  /// *false* the flush is scheduled and returns immediately.
  static Future<void> flush({bool isSync = false}) {
    return _xLogPlugin.flush(isSync);
  }
...
class _FlutterXLogPlugin extends PlatformInterface {
  ...
  Future<void> flush(bool isSync) {
    return methodChannel.invokeMethod<void>('flush', {'isSync': isSync});
  }
}
